...
var _validator, _encryptionKey, _options, _defaultValues, _a;
...
function createMiniWindow() {
  ...
  miniWindow = new electron.BrowserWindow({
    width: 280,
    height: 120,
    x: width - 300,
    y: 20,
  ...
  const miniUrl = `${"http://localhost:5173"}?mini=true`;
  miniWindow.loadURL(miniUrl);
  miniWindow.once("ready-to-show", () => {
    miniWindow == null ? void 0 : miniWindow.show();
  });
  miniWindow.on("closed", () => {
    miniWindow = null;
  });
  miniWindow.on("ready-to-show", () => {
    miniWindow == null ? void 0 : miniWindow.webContents.executeJavaScript(`
...
function updateTrayMenu(timeText, mode) {
  if (!tray) return;
  ...
  const contextMenu = electron.Menu.buildFromTemplate([
    ...
    { type: "separator" },
    {
      ...
      click: () => {
        if (mode === "running") {
          const mainWindow2 = require("./windows").getMainWindow();
          if (mainWindow2) {
            mainWindow2.webContents.send("tray:pause");
          }
        } else if (mode === "paused") {
          const mainWindow2 = require("./windows").getMainWindow();
          if (mainWindow2) {
            mainWindow2.webContents.send("tray:resume");
          }
        } else {
          const mainWindow2 = require("./windows").getMainWindow();
          if (mainWindow2) {
            mainWindow2.webContents.send("tray:start-last");
          }
        }
      }
    },
    {
      ...
      click: () => {
        const mainWindow2 = require("./windows").getMainWindow();
        if (mainWindow2) {
          mainWindow2.webContents.send("tray:stop");
        }
      }
    },
    { type: "separator" },
...
function updateTrayTimer(remainingMs, mode) {
...
const tasksStore = new ElectronStore({
  name: "tasks",
  defaults: {
    tasks: []
  }
});
const sessionsStore = new ElectronStore({
  name: "sessions",
  defaults: {
    sessions: []
  }
...
const taskOperations = {
  ...
  update(id2, updates) {
    const tasks = this.getAll();
    const index = tasks.findIndex((t2) => t2.id === id2);
  ...
  delete(id2) {
    const tasks = this.getAll();
    const filtered = tasks.filter((t2) => t2.id !== id2);
...
const sessionOperations = {
  ...
  update(id2, updates) {
    const sessions = this.getAll();
    const index = sessions.findIndex((s) => s.id === id2);
    if (index === -1) return null;
...
const storeUtils = {
  ...
  getTodayWorkSessionCount(taskId) {
...
function startTimer(taskId, kind, minutes) {
  if (timerInterval) {
  ...
  timerInterval = setInterval(() => {
    const now2 = Date.now();
    const remaining = timerState.endsAt - now2;
    if (remaining <= 0) {
...
function stopTimer() {
  ...
  timerState = {
    ...
    remainingMs: 0,
    plannedMinutes: null,
    completedWorkCount: timerState.completedWorkCount
    // Keep the count
  };
...
function setupIpcHandlers() {
  electron.ipcMain.handle("timer:start", async (_, params) => {
    startTimer(params.taskId, params.kind, params.minutes);
  });
  electron.ipcMain.handle("timer:pause", async () => {
    pauseTimer();
  });
  electron.ipcMain.handle("timer:resume", async () => {
    resumeTimer();
  });
  electron.ipcMain.handle("timer:stop", async () => {
    stopTimer();
  });
  electron.ipcMain.handle("timer:getState", async () => {
    return timerState;
  });
  electron.ipcMain.handle("tasks:getAll", async () => {
    return taskOperations.getAll();
  });
  electron.ipcMain.handle("tasks:create", async (_, task) => {
    return taskOperations.create(task);
  });
  electron.ipcMain.handle("tasks:update", async (_, id2, updates) => {
    return taskOperations.update(id2, updates);
  });
  electron.ipcMain.handle("tasks:delete", async (_, id2) => {
    taskOperations.delete(id2);
  });
  electron.ipcMain.handle("sessions:getToday", async (_, taskId) => {
    return sessionOperations.getToday(taskId);
  });
  electron.ipcMain.handle("sessions:getAll", async () => {
...
setupIpcHandlers();
electron.ipcMain.handle("app:toggleMini", async () => {
  toggleMiniWindow();
});
...
function registerGlobalShortcuts() {
  try {
    electron.globalShortcut.register("CmdOrCtrl+Shift+T", () => {
      const mainWindow2 = electron.BrowserWindow.getAllWindows().find((w) => !w.isDestroyed());
      if (mainWindow2) {
        mainWindow2.webContents.send("shortcut:toggle-timer");
      }
    });
    electron.globalShortcut.register("CmdOrCtrl+Shift+M", () => {
      toggleMiniWindow();
    });
    console.log("Global shortcuts registered successfully");
  } catch (error2) {
    console.error("Failed to register global shortcuts:", error2);
  }
}
function setupTrayUpdates() {
  const handleTimerUpdate = (event, data) => {
  ...
  electron.app.on("browser-window-created", updateTrayListener);
}
